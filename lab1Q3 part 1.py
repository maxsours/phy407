...
@author: Anntara Khan(1002321891), Max Sours
"""
import numpy as np
...
'''
N_range = np.arange(0,200) #array to plot the y-axis

T = [] #list to plot the x-axis
for N in N_range:
    ...
    T.append(diff1)
plt.figure(1)
plt.plot(N_range,T) #plots the graph
plt.title('N over Time (matrix multiply)')
plt.xlabel('N')
plt.ylabel('Time')
plt.figure(2)
plt.plot(N_range**3,T) #plots the graph
